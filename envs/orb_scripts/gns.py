...
class MoleculeGNS(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.intermediate_node_embeddings = []
    def forward(self, batch: base.AtomGraphs) -> Dict[str, torch.Tensor]:
        ...
        """
        self.intermediate_node_embeddings = [] # Initialize list to store encoder output  
      
        # Featurize inputs  
        edge_features = self.featurize_edges(batch)  
        node_features = self.featurize_nodes(batch)  
        
        # Store initial node features (before encoder)  
        self.intermediate_node_embeddings.append(node_features.detach().mean(dim=0))  
        
        # Encode  
        nodes, edges = self._encoder(node_features, edge_features)  
        
        # Store encoder output  
        self.intermediate_node_embeddings.append(nodes.detach().mean(dim=0))  
