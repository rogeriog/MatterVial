...
def get_MVL_MEGNetFeatures(structures, layer_name='layer32'):
    ...
    suffix = 'MVL32' if layer_name == 'layer32' else 'MVL16'
    
    MVL_MEGNetFeats = []  # To store features from all models
    for model_name in ["Eform_MP_2019", 'Efermi_MP_2019', "Bandgap_classifier_MP_2018", 'Bandgap_MP_2018', 'logK_MP_2019', 'logG_MP_2019']:
        ...
        for original_index, s in zip(indexes, structures):
            try:
                ...
                graph = model.graph_converter.convert(s)
                inp = model.graph_converter.graph_to_input(graph)  ## isolate this later
                ...
                s_model_results = pd.DataFrame([pred[0][0]], 
                                               columns=[f"{suffix}_{model_name}_{idx+1}" for idx in range(len(pred[0][0]))],
                ...
                columns = [f"{suffix}_{model_name}_{idx+1}" for idx in range(pred.shape[2])]
...
def get_Custom_MEGNetFeatures(structures,
    ...
    for original_index, s in zip(indexes, structures):
        ...
        MEGNetFeatsDF.append(feat_data)
